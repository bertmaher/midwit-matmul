...
__global__ void mma_wgmma(
  ...
  wgmma(
      c_regs,
      desc((uint64_t)a_shared, 128, 256, 0, 0),
      desc((uint64_t)b_shared, 128 * 256 / 8, 128, 0, 0));
  wgmma_commit_group();
  wgmma_wait_group();
  ...
  if (tid == 0) {
    for (int m = 0; m < 64; m++) {
      for (int n = 0; n < 256; n++) {
        float p = 0.0;
        for (int k = 0; k < 16; k++) {
          p += __bfloat162float(a_shared[m * 16 + k]) *
              __bfloat162float(b_shared[n * 16 + k]);
        }
